...
// #include "~/open_source_code/OpSparse-main/spECK/cub-1.17.2/cub/warp/specializations/warp_scan_shfl.cuh"
// #include "./cub-1.17.2/cub/warp/specializations/warp_scan_shfl.cuh"
...
// 	unsigned int* d_aptr,unsigned int* d_acol,size_t nums_Arow, size_t nnz_A,unsigned int* d_bptr,unsigned int *d_bcol, size_t nums_Brow,size_t nnz_B,
// 	uint32_t *out, uint32_t rowsPerBlock, 
// 	uint32_t *maxComputationsPerRow, uint32_t *rowColMinMax, uint32_t *rowOperationsMax, uint32_t *sumProducts)
// {
// 	uint32_t startRow = blockIdx.x * rowsPerBlock;
// 	uint32_t lastRowExcl = min(uint32_t((blockIdx.x + 1) * rowsPerBlock), uint32_t(nums_Arow));
...
// 	__shared__ uint32_t rowOpsCounter[THREADS];
// 	__shared__ uint32_t rowOffsets[THREADS];
// 	__shared__ uint32_t rowMaxOps[THREADS];
// 	__shared__ uint32_t rowMinCols[THREADS];
// 	__shared__ uint32_t rowMaxCols[THREADS];
// 	__shared__ uint32_t blockProducts;
// 	__shared__ uint32_t blockMaxOps;
...
// 		uint32_t rowB = d_acol[id];
// 		uint32_t startIdB = d_bptr[rowB];
// 		uint32_t lastIdBExcl = rowB + 1 <= nums_Brow ? d_bptr[rowB + 1] : nnz_B;
// 		uint32_t operations = lastIdBExcl - startIdB;
...
// 	if (rowsPerBlock > 1) {
// 		uint32_t rowProducts = rowOpsCounter[threadIdx.x];
...
// 		uint32_t maxRowLength = rowOpsCounter[threadIdx.x];
// 		for (int i = 16; i > 0; i /= 2)
...
// 			// uint32_t check = toRowColMinMax((uint32_t)rowMinCols[threadIdx.x], (uint32_t)rowMaxCols[threadIdx.x]);
// 		if(checkRowOpsMax)

...
template<typename T>
CSR<T>::CSR(const CSR<T> &A, int r, int c, int row_start, int col_start){
    assert(r + row_start <= A.rows && "matrix subsect error M");
    assert(c + col_start <= A.cols && "matrix subsect error N");
    int row_end = row_start + r;
    int col_end = col_start + c;
    rows = r;
    cols = c;
    int *row_size = new int [rows];
    memset(row_size, 0, rows*sizeof(int));
    for(int i = row_start; i < row_end; i++){
        for(int j = A.row_offsets[i]; j < A.row_offsets[i+1]; j++){
            if(A.col_ids[j]>= col_start && A.col_ids[j] < col_end){
    ...
    row_offsets[0] = 0;
    for(int i = 0; i < rows; i++){
        row_offsets[i+1] = row_offsets[i] + row_size[i];
    }
    delete [] row_size;

    for(int i = row_start; i < row_end; i++){
        int jj = row_offsets[i - row_start];
        for(int j = A.row_offsets[i]; j < A.row_offsets[i+1]; j++){
            if(A.col_ids[j]>= col_start && A.col_ids[j] < col_end){
                col_ids[jj] = A.col_ids[j] - col_start;
                data[jj++] = A.data[j];
            }
        }
    }
...
template <typename T>
CSR<T>& CSR<T>::operator=(const CSR<T>& src){
    alloc(src.rows, src.cols, src.nnz);
	rows = src.rows; nnz = src.nnz; cols = src.cols;
	memcpy(data, src.data, nnz * sizeof(T));
	memcpy(col_ids, src.col_ids, nnz * sizeof(int));
	memcpy(row_offsets, src.row_offsets, (rows + 1) * sizeof(int));
    return *this;
}

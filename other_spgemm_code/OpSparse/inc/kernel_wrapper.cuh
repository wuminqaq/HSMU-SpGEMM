...
inline void h_numeric_full_occu(const CSR& A, const CSR& B, CSR& C, Meta& meta){
    ...
    if(meta.bin_size[6]){
        ...
        k_numeric_max_shared_hash_tb_half_occu<<<meta.bin_size[6], 1024, 98304, meta.stream[6]>>>
            ...
            #if compute_share
            practical_give += (unsigned long long)meta.bin_size[6]*98304;
            printf("meta.bin_size[6]*98304 is %ld\n",meta.bin_size[6]*98304);
            #endif
    }


    if(meta.bin_size[7]){ // global bin
        printf("the meta.bin_size[7] is %d\n",meta.bin_size[7]);
        ...
        k_numeric_global_hash_tb_full_occu<<<meta.bin_size[7], 1024, 0, meta.stream[7]>>>
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)global_size;
            printf("global_size is %ld\n",global_size);
            #endif
    }
    ...
    if(meta.bin_size[5]){
        k_numeric_shared_hash_tb_full_occu<4096, 1024>
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)meta.bin_size[5]*4096*12;
            printf("meta.bin_size[5]*4096*12 is %ld\n",meta.bin_size[5]*4096*12);
            #endif
    }
    if(meta.bin_size[4]){
        k_numeric_shared_hash_tb_full_occu<2048, 512>
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)meta.bin_size[4]*2048*12;
            printf("meta.bin_size[4]*2048*12 is %ld\n",meta.bin_size[4]*2048*12);
            #endif
    }
    if(meta.bin_size[3]){
        k_numeric_shared_hash_tb_full_occu<1024, 256>
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)meta.bin_size[3]*1024*12;
            printf("meta.bin_size[3]*1024*12 is %ld\n",meta.bin_size[3]*1024*12);
            #endif
    }
    if(meta.bin_size[2]){
        // printf("the meta.bin_size[2] is %d\n",meta.bin_size[2]);
        k_numeric_shared_hash_tb_full_occu<512, 128>
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)meta.bin_size[2]*512*12;
            printf("meta.bin_size[2]*512*12 is %ld\n",meta.bin_size[2]*512*12);
            #endif
    }

    if(meta.bin_size[1]){
        // printf("the meta.bin_size[1] is %d\n",meta.bin_size[1]);
        k_numeric_shared_hash_tb_full_occu<256, 64>
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)meta.bin_size[1]*256*12;
            printf("meta.bin_size[1]*256*12 is %ld\n",meta.bin_size[1]*256*12);
            #endif
    }
    if(meta.bin_size[0]){
        // printf("the meta.bin_size[0] is %d\n",meta.bin_size[0]);
        ...
        k_numeric_shared_hash_pwarp<<<GS, BS, 0, meta.stream[0]>>>(
            ...
            C.d_rpt, C.d_col, C.d_val);
            #if compute_share
            practical_give += (unsigned long long)GS*NUMERIC_PWARP_ROWS * NUMERIC_PWARP_TSIZE*12;
            printf("GS*NUMERIC_PWARP_ROWS * NUMERIC_PWARP_TSIZE*12 is %ld\n",GS*NUMERIC_PWARP_ROWS * NUMERIC_PWARP_TSIZE*12);
            #endif
    }
    ...
    #if compute_share
    printf(" the practical_give is % lld\n",practical_give);
    printf(" the true_need is % lld\n",true_need);
    double use_of_shared = (double)((double)true_need/(double)practical_give);
    printf(" the use_of_shared is % lf\n",use_of_shared);
    
    FILE *fout_mem = fopen("./data/OpSparse_new_repres_use_of_share.csv", "a");
    if (fout_mem == NULL)
